...
import { deleteElement, resetRemainingDays, saveList, setComplete } from "../redux/actions";
...
export default function Todo({ Todo }) {
    ...
    const getStatusColor = () => {
        const  remainingDays = Todo.remainingDays;
        if (remainingDays<=2) {
            return {backgroundColor:'#ff7676',color:'white'}
        }else if (remainingDays>2 && remainingDays<5) {
            return {backgroundColor:'#edb95e'}
        }else{
            return {};
        }
    }
    ...
    return (
        <div className={ClassAction} style={getStatusColor()}>
            ...
            <div className="tododivright">
                <div style={{ marginRight: "10px" }}>
                    {Math.floor(Todo.remainingDays)} days
                </div>
