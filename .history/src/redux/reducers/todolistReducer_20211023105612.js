...
const TodoListReducer = (state = initialState, action) => {
    ...
    switch (action.type) {
        ...
        case "EDIT_ELEMENT":
            ...
            return tmp;
        case "RESET_REMAININGDAYS":
            tmp.forEach((e) => {
                if (e.id === action.data.id) {
                    const date2 = new Date(10/29/2021);
                    const date1 = new Date(e.time);
                    const Difference_In_Time = date1.getTime() - date2.getTime();
                    const Difference_In_Days = Difference_In_Time / (1000 * 3600 * 24);
                    e.remainingDays = Difference_In_Days;
                }
            });
            return tmp;
        case "SAVE":
            localStorage.setItem("listTodo", JSON.stringify(tmp));
            return tmp;
        case "RESET":
            return [];
